...
if ! lscpu | grep -q 'avx'; then
  msg_warn "No AVX detected: Installing MongoDB 4.4 (compatible with non-AVX systems)"
else
...
# Fallback if the grep pattern doesn't work
if [ -z "$OMADA_URL" ]; then
  # Try alternative pattern for newer filename format
  OMADA_URL=$(curl -fsSL "https://support.omadanetworks.com/en/download/software/omada-controller/" |
