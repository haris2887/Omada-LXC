...
build_container() {
  ...
  for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
      msg_error "LXC Container failed to start properly"
      exit 1
    fi
    sleep 2
  done

  # Wait for container init process to be ready
  msg_info "Waiting for container init process"
  for i in {1..20}; do
    if pct exec "$CTID" -- test -f /sbin/init >/dev/null 2>&1; then
      msg_ok "Container init process ready"
      break
    fi
    if [ $i -eq 20 ]; then
      msg_error "Container init process not ready"
      exit 1
    fi
    sleep 3
  done

  # Additional wait for network and services
  ...
  for i in {1..15}; do
    ...
    if [ $i -eq 15 ]; then
      msg_warn "Container network may not be ready, continuing anyway"
    fi
    sleep 5
  done
